...
for num_shots in cfg.number_shots:
    for iteration in range(cfg.iteration):
        print('Class number :', cfg.fs_class, 'with', num_shots, 'shots.')
        ...
        global_acc, fs_acc = t_u.basics_perf(
            testloader, model, device, classes, True)

        # Save model
        torch.save(model, path_res+'/model_'+str(num_shots)+'_'+str(iteration)+'.pth')
        ...
        if cfg.Finetune:
            ...
            torch.save(model, path_res+'/model_ft_'+str(num_shots)+'_'+str(iteration)+'.pth')
...
for k in dict_acc_int:
    dict_acc_int[k] = 1.96*np.sqrt(dict_acc_int[k])/np.sqrt(
        (cfg.iteration*len(cfg.number_shots)))
